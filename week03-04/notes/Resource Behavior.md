...
3. Kubernetes response
   ...
   Kubernetes tries to self-heal and recreate the failed pod. After repeated fails, it turns the status to CrashLoopBackOff which exponentially delays the tries (10s, 20, 40s,...) to avoid wasting CPU cycles trying to execute a container in a pod that is broken.
